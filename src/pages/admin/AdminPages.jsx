...
export const AdminOrders = () => {
  ...
  const load = async () => setOrders(await adminService.listOrders());
  ...
  return <div className="py-8 space-y-3"><h1 className="section-title">Orders</h1><div className={card}><div className="flex flex-wrap gap-2 items-center"><select value={filter} onChange={(e) => setFilter(e.target.value)} className="border p-2 rounded bg-black/20"><option value="">All statuses</option><option value="Pending">New orders</option>{statusList.map((s) => <option key={s}>{s}</option>)}</select><button onClick={() => setFilter('Pending')} className="px-3 py-2 rounded bg-charcoal text-white">View New</button></div></div>{filteredOrders.map((o) => <div key={o._id} className={card}><div className="flex justify-between gap-2 flex-wrap"><div><p>#{o._id}</p><p className="text-xs">{o.user?.name || `${o.user?.firstName || ''} ${o.user?.lastName || ''}`.trim()} • {o.paymentMethod} • ₹{o.total}</p><p className="text-xs">Current status: {o.status}</p></div><div className="flex gap-2 flex-wrap"><button onClick={() => updateStatus(o._id, 'Confirmed')} className="text-xs px-2 py-1 rounded border">Confirm</button><button onClick={() => updateStatus(o._id, 'Packed')} className="text-xs px-2 py-1 rounded border">Mark Packed</button><button onClick={() => updateStatus(o._id, 'Dispatched')} className="text-xs px-2 py-1 rounded border">Mark Dispatched</button><button onClick={() => updateStatus(o._id, 'Delivered')} className="text-xs px-2 py-1 rounded border">Mark Delivered</button><button onClick={() => updateStatus(o._id, 'Cancelled')} className="text-xs px-2 py-1 rounded border border-rose-300 text-rose-200">Cancel</button><select value={o.status} onChange={(e) => updateStatus(o._id, e.target.value)} className="border p-2 rounded bg-black/20">{statusList.map((s) => <option key={s}>{s}</option>)}</select></div></div>{o.items?.length ? <div className="mt-2 space-y-1">{o.items.map((item) => <p key={item._id || item.product?._id || item.productName} className="text-xs">{item.productName} × {item.quantity} = ₹{Number(item.price) * Number(item.quantity)}</p>)}</div> : null}</div>)}</div>;
};

export const AdminUsers = () => {
  const [users, setUsers] = useState([]);
  const load = async () => setUsers(await adminService.listUsers());
  useEffect(() => { load(); }, []);

  return <div className="py-8"><h1 className="section-title mb-3">Users</h1><div className={card}>{users.map((u) => <div key={u._id} className="border-b border-white/10 py-2 text-sm flex justify-between items-center"><span>{u.name || `${u.firstName || ''} ${u.lastName || ''}`.trim()} • {u.email} • {u.role} • Orders: {u.orderCount || 0}</span><button className="text-cyan-200" onClick={async () => { await adminService.toggleUserBlock?.(u._id, !u.isBlocked); load(); }}>{u.isBlocked ? 'Unblock' : 'Block'}</button></div>)}</div></div>;
};

export const AdminBanners = () => {
  const [rows, setRows] = useState([]);
  const [editingId, setEditingId] = useState('');
  const [form, setForm] = useState({ title: '', subtitle: '', cta: 'Shop Now', link: '/shop', order: 0, active: true, image: null });
  const load = async () => setRows(await adminService.listBanners());

  useEffect(() => { load(); }, []);

  const onSave = async () => {
    if (editingId) await adminService.updateBanner(editingId, form);
    else await adminService.createBanner(form);
    setEditingId('');
    setForm({ title: '', subtitle: '', cta: 'Shop Now', link: '/shop', order: 0, active: true, image: null });
    load();
  };
...
export const AdminCoupons = () => {
  const [rows, setRows] = useState([]);
  const [editingId, setEditingId] = useState('');
  const [form, setForm] = useState({ code: '', type: 'percentage', value: 10, minOrderAmount: 0, expiry: '', active: true });
  const load = async () => setRows(await adminService.listCoupons());

  useEffect(() => { load(); }, []);

  const save = async () => {
    const payload = { ...form, code: form.code.toUpperCase() };
    if (editingId) await adminService.updateCoupon(editingId, payload);
    else await adminService.createCoupon(payload);
    setEditingId('');
    setForm({ code: '', type: 'percentage', value: 10, minOrderAmount: 0, expiry: '', active: true });
    load();
  };
...
export const AdminCategories = () => {
  ...
  const load = async () => setRows(await adminService.listCategories());
...
export const AdminMedia = () => {
  ...
  const load = async () => setAssets(await adminService.listMedia());

  useEffect(() => { load(); }, []);

  const grouped = useMemo(() => assets.slice(0, 60), [assets]);

  return <div className="py-8"><h1 className="section-title mb-3">Uploaded Media</h1><div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">{grouped.map((asset) => <div key={asset._id} className={card}><img src={asset.secureUrl || asset.url} className="h-24 w-full object-cover rounded"/><button className="text-xs text-red-300 mt-2" onClick={async () => { await adminService.deleteMedia(asset.publicId); load(); }}>Delete</button></div>)}</div></div>;
};
