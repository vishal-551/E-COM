...
export const AdminOrders = () => {
  ...
  const [dispatch, setDispatch] = useState({ courier_name: '', tracking_id: '', dispatch_date: '', estimated_delivery_date: '' });
  const load = async () => {
  ...
  const updateStatus = async (order, status) => {
    await supabase.from('orders').update({ status, ...dispatch }).eq('id', order.id);
    load();
  };

  return <div className="py-8 space-y-3"><h1 className="section-title">Orders</h1><div className={card}><select value={filter} onChange={(e) => setFilter(e.target.value)} className="border p-2 rounded bg-black/20"><option value="">All statuses</option>{statusList.map((s) => <option key={s}>{s}</option>)}</select></div>{orders.map((o) => <div key={o.id} className={card}><div className="flex justify-between"><div><p>#{o.id}</p><p className="text-xs">{o.profile?.full_name} • {o.payment_method} • ₹{o.total_amount}</p></div><select value={o.status} onChange={(e) => updateStatus(o, e.target.value)} className="border p-2 rounded bg-black/20">{statusList.map((s) => <option key={s}>{s}</option>)}</select></div><div className="grid md:grid-cols-4 gap-2 mt-2">{['courier_name', 'tracking_id', 'dispatch_date', 'estimated_delivery_date'].map((k) => <input key={k} placeholder={k} value={dispatch[k]} onChange={(e) => setDispatch({ ...dispatch, [k]: e.target.value })} className="border p-2 rounded bg-black/20" />)}</div></div>)}</div>;
};
