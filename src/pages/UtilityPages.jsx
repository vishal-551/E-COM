...
export const AccountPage = () => { const me = JSON.parse(localStorage.getItem('aaj_user') || '{}'); return <div className="py-8"><h1 className="section-title">My Account</h1><p className="mt-2">Name: {me.name || 'Guest'} • Email: {me.email || 'Not logged in'}</p></div>; };
...
export const LoginPage = () => { const [email,setEmail]=useState(''); const [password,setPassword]=useState(''); return <div className="py-8 max-w-md mx-auto premium-card p-4"><h1 className="section-title">Login</h1><input value={email} onChange={(e)=>setEmail(e.target.value)} className="border p-2 rounded w-full my-2" placeholder="Email"/><input value={password} onChange={(e)=>setPassword(e.target.value)} type="password" className="border p-2 rounded w-full mb-2" placeholder="Password"/><button onClick={()=>{localStorage.setItem('aaj_token',`jwt-demo-${Date.now()}`);localStorage.setItem('aaj_user',JSON.stringify({name:'Demo User',email}));alert('Logged in')}} className="bg-charcoal text-white py-2 rounded w-full">Login</button></div>; };
export const SignupPage = () => { const [name,setName]=useState(''); const [email,setEmail]=useState(''); const [password,setPassword]=useState(''); return <div className="py-8 max-w-md mx-auto premium-card p-4"><h1 className="section-title">Signup</h1><input value={name} onChange={(e)=>setName(e.target.value)} className="border p-2 rounded w-full my-2" placeholder="Name"/><input value={email} onChange={(e)=>setEmail(e.target.value)} className="border p-2 rounded w-full my-2" placeholder="Email"/><input value={password} onChange={(e)=>setPassword(e.target.value)} type="password" className="border p-2 rounded w-full mb-2" placeholder="Password"/><button onClick={()=>{localStorage.setItem('aaj_user',JSON.stringify({name,email}));alert('Signup successful')}} className="bg-charcoal text-white py-2 rounded w-full">Create account</button></div>; };
