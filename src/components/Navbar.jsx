...
const links = [
  ['/', 'Home'], ['/shop', 'Shop'], ['/categories', 'Categories'], ['/new-arrivals', 'New Arrivals'], ['/best-sellers', 'Best Sellers'], ['/about', 'About'], ['/contact', 'Contact'],
];

export default function Navbar() {
  const { cart, wishlist, settings, user } = useStore();
    ...
    <>
      ...
      <header className="sticky top-0 z-40 bg-ivory/95 backdrop-blur border-b border-champagne">
        <nav className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
          <Link to="/" className="text-xl font-bold tracking-wide text-gold">{settings.storeName || 'E-COM'}</Link>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to={user ? '/profile' : '/login'}><User size={18} /></Link>
